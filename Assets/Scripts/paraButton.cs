...
public class paraButton : MonoBehaviour
{
    ...
    private void mySelect(SelectEnterEventArgs args)
    {
        ...
        if (panelIndex != -1)
        {
            panelScripts[panelIndex].PassValueToParaButton();
            debugText.text = p_sumOrPara.name + ", " + p_original.name;
        }
        p_sumOrPara.transform.position = p_original.transform.position;
        p_sumOrPara.transform.rotation = p_original.transform.rotation;
        p_sumOrPara.SetActive(true);
